...
class MemberService:
    ...
    async def invite_member(
        ...
        await self._log(invited_by.id, AuditAction.CREATE, "invitation", invited_by.id)
    ...
    async def get_member_by_id(self, member_id: str) -> MemberResponse:
        result = await self._db.execute(select(Member).where(Member.id == uuid.UUID(member_id)))
    ...
    async def delete_member(
        ...
        if member.role == Role.ADMIN:
            ...
            if admin_count <= 1:
                raise BusinessRuleError("Impossible de supprimer le seul administrateur actif")
    ...
    async def list_group_invites(
        ...
        query = select(GroupInvitation).where(GroupInvitation.created_by == created_by.id)
        ...
        return [GroupInviteResponse.from_model(inv, settings.frontend_url) for inv in invites]
    ...
    async def delete_group_invite(
        ...
        result = await self._db.execute(
            select(GroupInvitation).where(GroupInvitation.id == _uuid.UUID(invite_id))
        )
        ...
        if str(invite.created_by) != str(requested_by.id):
            if requested_by.role != Role.ADMIN:
                raise ForbiddenError("Vous ne pouvez supprimer que vos propres liens d'invitation")
    ...
    async def deactivate_group_invite(
        ...
        if str(invite.created_by) != str(requested_by.id):
            if requested_by.role != Role.ADMIN:
                raise ForbiddenError("Vous ne pouvez désactiver que vos propres liens d'invitation")
    ...
    async def register_from_group_invite(
        ...
        if not invite.is_active:
            raise BusinessRuleError("Ce lien d'invitation a été désactivé par l'administrateur.")
