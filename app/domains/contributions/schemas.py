...
class UpdateAssociationSettingsRequest(BaseModel):
    ...
    def model_post_init(self, __context) -> None:
        if self.contribution_mode == "fixed" and not self.fixed_amount:
            raise ValueError("Le montant fixe est obligatoire en mode 'fixed'")
        if self.contribution_mode not in {"free", "fixed"}:
            raise ValueError("Mode invalide. Valeurs acceptées : 'free' ou 'fixed'")
...
class ContributionResponse(BaseModel):
    ...
    def from_model(
        ...
        return cls(
            ...
            expected_amount=float(contribution.expected_amount)
            if contribution.expected_amount
            else None,
            ...
            confirmed_at=contribution.confirmed_at.isoformat()
